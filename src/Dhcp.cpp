...
void DhcpClass::setHostName(const char* hostNameSet)
{
	_host_name = hostNameSet;
}
...
void DhcpClass::send_DHCP_MESSAGE(uint8_t messageType, uint16_t secondsElapsed)
{
	...
	*/
		// OPT - host name
	buffer[16] = hostName;
	buffer[17] = strlen(_host_name) + 1; // length of hostname + last 3 bytes of mac address
	strcpy((char*)&(buffer[18]), _host_name);
	_dhcpUdpSocket.write(buffer, (strlen(_host_name) + 20));
